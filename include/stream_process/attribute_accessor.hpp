...
public:
    ...
    attribute& setup_attribute( point_structure& ps,
		const std::string& name, bool is_output_, size_t array_size = 0 );

    attribute& setup_attribute( const point_structure& ps, 
...
operator()( stream_data* stream_data_ )
{
...
inline const T&
attribute_accessor< T >::
operator()( const stream_data* stream_data_ ) const
{
	assert( _offset != std::numeric_limits< size_t >::max() );
	assert( stream_data_ );
...
inline attribute&
attribute_accessor< T >::
setup_attribute( point_structure& ps, const std::string& name, bool is_output_,
    size_t array_size )
{
	attribute* attr = 0;
...
inline attribute&
attribute_accessor< T >::
setup_attribute( const point_structure& ps, const std::string& name )
{
	attribute* attr = 0;
	if ( ! ps.has_attribute( name ) )
	{
...
const T&
attribute_accessor< T >::
get_with_offset( size_t offset_, const stream_data* data_ )
{
