...
#define SP_CLASS_NAME optimal_transform< vertex_t, vertex_accessor_t, container_t > 

SP_TEMPLATE_STRING
bool
SP_CLASS_NAME::
analyze( const container_t& mesh_ )
{
    std::cout << "optimal_transform: computing transformation matrix." << std::endl;
    ...
    vmml::solve_jacobi_3x3< double >( _covariance, eigenvalues, eigenvectors, rotation_count );
    
    size_t largest_eigenvalue_index = eigenvalues.find_max_index();
    
    mat3d rotation;    

    double r0[] = { 0, 0, -1, 0, 1, 0, 1, 0, 0 };
    double r1[] = { 1, 0, 0, 0, 0, -1, 0, 1, 0 };
...
void
SP_CLASS_NAME::
_compute_covar( const container_t& mesh_ )
{
...
void
SP_CLASS_NAME::
set_transformation_matrix( const mat4& transform_ )
{
...
SP_TEMPLATE_STRING
const typename SP_CLASS_NAME::mat4&
SP_CLASS_NAME::
get_transformation_matrix()
{
    for( size_t i = 0; i < 4; ++i )
    {
        ...
        {
            _transform_user_type( i, j ) = 
                static_cast< value_type >( _transform( i, j ) );
        }
    }
