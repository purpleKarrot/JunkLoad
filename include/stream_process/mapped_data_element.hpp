...
mapped_data_element::operator[]( size_t index )
{
    ...
    return reinterpret_cast< stream_data* >( 
        data() + index * _element.get_size_in_bytes()
        );
}
...
mapped_data_element::operator[]( size_t index ) const
{
    assert( index < _element.get_size() );
    assert( _mapped_file.data() );
    return reinterpret_cast< const stream_data* >( 
        data() + index * _element.get_size_in_bytes()
        );
}
...
inline mapped_data_element::iterator
mapped_data_element::begin()
{
...
inline mapped_data_element::const_iterator
mapped_data_element::begin() const
{
...
inline bool
mapped_data_element::is_open() const
{
...
inline size_t
mapped_data_element::size() const
{
