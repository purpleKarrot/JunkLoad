#ifndef __STREAM_PROCESS__NORMAL_ORIENT_HACK_OP_ALGORITHM__HPP__
#define __STREAM_PROCESS__NORMAL_ORIENT_HACK_OP_ALGORITHM__HPP__
...
class normal_orient_hack_op_algorithm : public op_algorithm
{
public:
    typedef vmml::vector< 3, float_t >      vec3;
    
    normal_orient_hack_op_algorithm();
...
template< typename float_t, typename integer_t >
normal_orient_hack_op_algorithm< float_t, integer_t >::
normal_orient_hack_op_algorithm()
    : _normal(      "normal" )
...
clone()
{
    return new normal_orient_hack_op_algorithm< float_t, integer_t >( *this );
}
...
about() const
{
    return "normal orientation hack";
}
...
_compute( stream_point* point )
{
    ...
    {
        if ( current_nb->get_point()->get( _point_index ) < point_index )
        {
            xn += current_nb->get_point()->get( _normal );
            inv_test = true;
        }
    }
    if ( ! inv_test )
