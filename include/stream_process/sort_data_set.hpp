...
#include <stream_process/preprocess_types.hpp>
#include <stream_process/special_accessors.hpp>
...
protected:
    ...
    template< typename value_t, typename index_t >
    void _sort_other( const std::string& attribute_name );
    ...
    void _sort_file( const value_accessor_t& accessor_, size_t size_, 
...
void
sort_data_set::
_sort_other( const std::string& attribute_name )
{
    // FIXME
    throw exception( "SORT OTHER NOT IMPLEMENTED YET.", SPROCESS_HERE );
}
...
void
sort_data_set::
_sort_file( const value_accessor_t& accessor, size_t number_of_elements, 
    const std::string& file_suffix )
{
#if 0
    ...
    params.path             = _params.tmp_name;
    params.new_file_size    = sizeof( sort_ref ) * number_of_elements;
    params.mode             = std::ios_base::in | std::ios_base::out;
    
    _tmp_file.open( params );
    ...
    {
        sort_ref& ref               = *it;
        const stream_data* point    = _input[ index ];
    ...
    // setup result file
    const size_t in_point_size
        = _input.get_header().get_vertex_structure().compute_size_in_bytes();
    
    params.path             = _params.out_name + file_suffix;
    params.new_file_size    = number_of_elements * in_point_size;
    
    _out_file.open( params );
    ...
    {
        sort_ref& sr                = *it;
        const stream_data* source   = _input[ sr.index ];

        memcpy( out_data, source, in_point_size );
    }
...
void
sort_data_set::
_sort_file( const mapped_data_element& source_,
