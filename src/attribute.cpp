...
void
attribute::set_data_type_id( data_type_id id_ )
{
    _data_type_id = id_;
}
...
size_t
attribute::get_element_size_in_bytes() const
{
...
size_t
attribute::get_number_of_elements() const
{
...
size_t
attribute::get_size_in_bytes() const
{
...
void
attribute::set_offset( size_t offset_ )
{
    _offset = offset_;
}
...
void
attribute::set_number_of_elements( size_t number_of_elements_ )
{
...
void
attribute::set_is_high_precision( bool is_hp )
{
...
bool
attribute::is_high_precision() const
{
...
void
attribute::set_is_output( bool is_output_ )
{
...
bool
attribute::from_header_string( const std::string& attr_string )
{
...
bool
attribute::from_header_string_vector( const std::vector< std::string >& tokens )
{
    if ( tokens.size() < 3 )
        return false;
    
    if ( tokens[ 0 ] == "vertex_attribute" 
        || tokens[ 0 ] == "face_attribute" 
        || tokens[ 0 ] == "attribute"
        || tokens[ 0 ] == "data"
        )
    {}
...
std::string
attribute::to_header_string() const
{
...
std::string
attribute::to_header_string( const std::string& identifier ) const
{
    const data_type_helper& dth = data_type_helper::get_singleton();
