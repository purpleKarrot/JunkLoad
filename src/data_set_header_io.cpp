...
void
data_set_header_io::read_from_file( const std::string& filename_base, 
...
void
data_set_header_io::write_to_file( const std::string& filename_base,
    const data_set_header& hdr_  )
{
    ...
    if ( transform_ != mat4d::IDENTITY )
    {
        ...
        for( size_t row = 0; row < 4; ++row )
        {
            ...
            {
                _fstream
                    << boost::lexical_cast< std::string, double >( transform_( row, col) )
                    << " ";
            }
        }
        _fstream << "\n\n";
...
void
data_set_header_io::_open_file( const std::string& filename_base,
    std::fstream::openmode mode_  )
{
    ...
    if ( ! _fstream.is_open() )
    {
        ...
        throw exception( 
            std::string( "opening point header " ) + _filename + 
                " failed.",
            SPROCESS_HERE );
    }
}
...
void
data_set_header_io::_parse_header()
{
    std::string line;
    size_t offset;
...
bool
data_set_header_io::_parse_line( std::deque< std::string >& tokens )
{
    ...
    try
    {
        ...
        if ( tokens[ 0 ] == "min" )
        {
            ...
            {
                aabb[ index ]
                    = boost::lexical_cast< double >( tokens[ index + 1 ] );
            }
        ...
        if ( tokens[ 0 ] == "transform" )
        {
            ...
            for( size_t row = 0, index = 1; row < 4; ++row )
            {
                ...
                {
                    transform_( row, col ) 
                        = boost::lexical_cast< double >( tokens[ index ] );
                }
            }
