...
void
mapped_data_element::open( const std::string& filename_base,
    bool create_new_file )
{
    ...
    {
        throw exception( 
            std::string( "opening file '" ) + params_.path 
                + "' failed.",
            SPROCESS_HERE
            );
    }
    ...
    {
        std::cout << "mapped file '" << params_.path << "' with a size" 
            << " of " << _mapped_file.size() << " bytes."
            << std::endl;
    }
    #endif
}
