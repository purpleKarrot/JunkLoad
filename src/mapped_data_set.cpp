...
void
mapped_data_set::_setup( bool new_file )
{
    try 
    {
...
size_t
mapped_data_set::get_vertex_size_in_bytes() const
{
...
size_t
mapped_data_set::get_face_size_in_bytes() const
{
...
std::string
mapped_data_set::build_info_string() const
{
    ...
    ss << "'" << fn_no_path << "'\n\n"
        ...
        << "Vertex Data\n"
        << "  - " << (float) _vertices.get_file_size_in_bytes() / 1048576.0
            << " MBytes\n"
        << "  - " << (float)_vertices.size() * 1e-6 << " Mio Vertices\n"
    ...
    if ( _faces.size() )
    {
        ss
            ...
            << "  - " << (float) _faces.get_file_size_in_bytes() / 1048576.0
                << " MBytes\n"
            << "  - " << (float)_faces.size() * 1e-6 << " Mio Faces\n"
            << "  - " << _vertices.get_size_in_bytes() << " Bytes/Face\n"
            <<"\n"
            << "Face Attributes\n";
