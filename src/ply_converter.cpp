    ...
    , _data_set( 0 )
{
    ...
    if ( boost::algorithm::iends_with( _params.source_file, ".ply" ) )
        ...
        throw exception( "ply-export not implemented yet.", 
            SPROCESS_HERE );
    else
...
void
ply_converter::_setup_header_from_vertex_properties( data_set_header& header )
{
    ...
    for( int index = 0; index < _vertex_property_count; ++index )
    {
        ...
        else if ( name == "a" || name == "alpha" )
        {
            ...
            _other_properties.push_back( std::pair< std::string, size_t >( name, index ) );
    }
    ...
    std::vector< std::pair< std::string, size_t > >::iterator 
        opit        = _other_properties.begin(),
        opit_end    = _other_properties.end();
...
void
ply_converter::_setup_header_from_face_properties( data_set_header& header )
{
    ...
    for( int index = 0; index < _face_property_count; ++index )
    {
        ...
        if ( index_name == prop->name )
        {
            fs.create_attribute(
                "vertex_indices", 
                SP_UINT_32,     // TODO make configable, or uint64_t?
                3 // triangles only
            );
        }
    }
...
void
ply_converter::_read_vertex_data()
{
    ...
    if ( vs.has_attribute( "color" ) )
    {
        ...
        ply_props.push_back( _create_ply_property( _color_names[ 0 ],
            PLY_UCHAR, offset ) );
        offset += sizeof( uint8_t );
        ply_props.push_back( _create_ply_property( _color_names[ 1 ],
            PLY_UCHAR, offset ) );
        offset += sizeof( uint8_t );
        ply_props.push_back( _create_ply_property( _color_names[ 2 ],
            PLY_UCHAR, offset ) );
    ...
    std::vector< PlyProperty >::iterator
        it      = ply_props.begin(),
        it_end  = ply_props.end();
    for( ; it != it_end; ++it )
    {
...
ply_converter::_read_face_data()
{
    ...
    PlyProperty face_property = { "vertex_indices", PLY_INT, PLY_INT, 
        offsetof( tmp_face, vertices ), 1, PLY_UCHAR, PLY_UCHAR, 
        offsetof( tmp_face, number_of_vertices ) };
    ...
    for( ; fit != fit_end; ++fit )
    {
        ...
        {
            throw exception( "found non-triangle face in ply.", 
                SPROCESS_HERE );
        }
    }
}
...
PlyProperty
ply_converter::_create_ply_property( const std::string& name, int internal_type, 
...
void
ply_converter::_init_ply_sp_type_map()
{
/*
